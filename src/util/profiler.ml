...
(* (internal procedure) *)
let record_arguments name args =
  let v =
...
let max_check = if record_max_time then max_check else fun _ _ _ -> ()

(* (internal procedure) *)
let count_one name =
  if BatMap.mem name !count_log then
    BatMap.find name !count_log := Int64.succ !(BatMap.find name !count_log)
  else count_log := BatMap.add name (ref Int64.one) !count_log

let count_one = if record_calling_frequency then count_one else fun _ -> ()

(* (internal procedure) *)
let update_log name interval =
  let v =
...
let start_logger () = log_start := Sys.time ()
...
let make_filename () =
  ...
  let rec make_file_i : int -> string =
   fun i -> if Sys.file_exists (name i) then make_file_i (i + 1) else name i
  in
  make_file_i 0
