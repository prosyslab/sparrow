...
let dual_formatter fmt1 fmt2 =
  ...
  F.pp_set_formatter_out_functions fmt
    { F.out_string = (fun s p n ->
          out_fun1.out_string s p n; out_fun2.out_string s p n);
      out_indent = (fun n ->
          out_fun1.out_indent n; out_fun2.out_indent n);
      out_flush = (fun () ->
          out_fun1.out_flush (); out_fun2.out_flush ());
      out_newline = (fun () ->
          out_fun1.out_newline (); out_fun2.out_newline ());
...
let flush = function
  | Some logger ->
    F.pp_print_flush logger.file (); F.pp_print_flush logger.dual ()
  | _ -> ()
...
let log to_consol logger lv =
  ...
  | Some logger when compare_level !level lv ->
    if to_consol then
      F.fprintf logger.dual
    else
      F.fprintf logger.file
  | _ ->
    F.ifprintf F.err_formatter
