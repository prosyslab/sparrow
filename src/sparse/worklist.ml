  ...
  module Workorder = struct
    ...
    let make g =
      ...
      let add_edge src dst (ng, i2n, n2i) =
        ...
        (NGraph.add_edge ng i_src i_dst, i2n, n2i)
      in
      let ng, i2n, _ =
        DUGraph.fold_edges add_edge g
          (NGraph.empty (DUGraph.nb_node g), BatMap.empty, BatMap.empty)
      in
      (ng, i2n)
    ...
    let projection scc ng =
      ...
      in
      Hashtbl.fold
        (fun x _ g -> add_back_edge x g)
        scc
        (NGraph.empty (Hashtbl.length scc))
  ...
  module Ord = struct
    ...
    let compare ((o1, head1), v1) ((o2, head2), v2) =
      ...
      if cmp_o = 0 then
        ...
        if c = 0 then if head1 = head2 then 0 else if head1 then -1 else 1
        else c
      else cmp_o
  end
