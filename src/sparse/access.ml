...
module Make (Dom : MapDom.CPO) = struct
  ...
  let find_proc pid t =
    try ProcMap.find pid t.access_proc with _ -> Info.empty

  (* abstract locations exclusively accessed in the given function *)
  let find_proc_local pid t =
    try ProcMap.find pid t.access_proc_local with _ -> PowLoc.empty

  let find_program_local t = t.access_program_local

  let find_proc_reach pid t =
    try ProcMap.find pid t.access_proc_reach with _ -> Info.empty

  let find_proc_reach_wo_local pid t =
    try ProcMap.find pid t.access_proc_reach_wo_local with _ -> Info.empty

  let find_def_nodes x t =
    try LocMap.find x t.def_nodes with _ -> PowNode.empty

  let find_use_nodes x t =
    try LocMap.find x t.use_nodes with _ -> PowNode.empty
