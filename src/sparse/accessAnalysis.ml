...
module Make (Sem : AccessSem.S) = struct
  ...
  let init_access_proc_reach pids callgraph access =
    list_fold
      (fun pid ->
  ...
  let init_access_proc_local access =
    ...
    let update_loc2proc pid acc_of_pid (loc2proc, nonlocals) =
      ...
      PowLoc.fold (update_loc2proc_1 pid) locs (loc2proc, nonlocals)
    in
    let loc2proc, _ =
      Access.fold_proc update_loc2proc access (LocMap.empty, PowLoc.empty)
    in
    let add_loc_pid loc pid = Access.add_proc_local pid loc in
    LocMap.fold add_loc_pid loc2proc access

  let init_access_program_local access =
    let local =
      Access.fold_proc_local
        (fun _ local acc -> PowLoc.union acc local)
        access PowLoc.empty
    in
    Access.add_program_local local access

  let init_defs_of access =
    Access.fold
      (fun node info access ->
        PowLoc.fold
          (fun loc access ->
  ...
  let perform global locset sem mem =
    ...
    |> init_access_program_local |> init_defs_of |> init_uses_of
end
