...
module Make (DUGraph : Dug.S) = struct
  ...
  let get_ordinary_uses access node =
    ...
    with _ ->
      let locset =
        if Global.is_rec pid global && not !Options.unsound_recursion then
          ...
          let uses =
            Access.Info.useof (Access.find_proc_reach_wo_local pid access)
          in
          PowLoc.inter locset uses
      in
    ...
    with _ ->
      let locset =
        if Global.is_rec pid global && not !Options.unsound_recursion then
          ...
          let defs =
            Access.Info.defof (Access.find_proc_reach_wo_local pid access)
          in
          PowLoc.inter locset defs
      in
      ...
      if InterCfg.is_returnnode node global.icfg then
        ...
        let defs_of_callees =
          let add_defs callee =
            PowLoc.union (defs_of_function global access callee locset)
          in
          ProcSet.fold add_defs callees PowLoc.empty
        in
        let add_not_defined callee =
          let defs = defs_of_function global access callee locset in
          PowLoc.union (PowLoc.diff defs_of_callees defs)
        in
    ...
    let rec iterate_node w var itercount hasalready work joinpoints =
      ...
      | node :: rest ->
          let rest, hasalready, work, joinpoints =
            IntraNodeSet.fold
              (fun y (rest, hasalready, work, joinpoints) ->
                if IntraNodeMap.find y hasalready < itercount then
                  let joinpoints =
                    map_set_add (Node.make pid y) var joinpoints
                  in
      ...
      | v :: rest ->
          ...
          let w, work =
            IntraNodeSet.fold
              ...
              ([], work)
          in
          let hasalready, work, joinpoints =
            iterate_node w v itercount hasalready work joinpoints
          in
  ...
  let to_json_intra g access =
    let nodes =
      `List
        (DUGraph.fold_node
           (fun v nodes -> `String (Node.to_string v) :: nodes)
           g [])
    in
    let edges =
      `List
        (DUGraph.fold_edges
           (fun src dst edges ->
             ...
             if spid = dpid then
               ...
               let localset =
                 PowLoc.filter (fun x -> Access.Info.mem x access_proc) addrset
               in
