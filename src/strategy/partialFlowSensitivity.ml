...
and is_sizeof e =
  match e with SizeOf _ | SizeOfE _ | SizeOfStr _ -> true | _ -> false

and is_var e = match e with Lval (Var _, NoOffset) -> true | _ -> false
...
let dec_itself (lv, e) =
  match (lv, e) with
  | (Var x, NoOffset), BinOp (MinusA, Lval (Var y, NoOffset), _, _) when x = y
    ->
      true
  | _ -> false
...
let extract1 icfg mem global node feature =
  ...
  try
    ...
    |> (match cmd with
       ...
       | Calloc (lv, IntraCfg.Cmd.Array _, _, _, _) ->
...
let closure global feature =
  ...
  let pred_set g s =
    PowLoc.fold (fun n -> PowLoc.union (pred g n)) s PowLoc.empty
  in
  let succ_set g s =
    PowLoc.fold (fun n -> PowLoc.union (succ g n)) s PowLoc.empty
  in
  let rec clos_backward set =
    let preds = pred_set copy_graph set in
    if PowLoc.subset preds set then set
    else clos_backward (PowLoc.union set preds)
  in
