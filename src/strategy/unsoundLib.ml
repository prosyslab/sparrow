...
let string_of_feature f =
  ...
  ^ string_of_bool f.from_string
  ^ "\n" ^ "inside_loop : "
  ^ string_of_bool f.inside_loop
  ^ "\n" ^ "update_param_in_loop : "
  ^ string_of_bool f.update_param_in_loop
  ^ "\n" ^ "use_ret_in_loop : "
  ^ string_of_bool f.use_ret_in_loop
  ^ "\n" ^ "update_param_itself : "
  ^ string_of_bool f.update_param_itself
  ^ "\n" ^ "out_of_fun : "
  ^ string_of_bool f.out_of_fun
...
let string_of_raw_feature f =
  b2is f.constant ^ "\t" ^ string_of_float f.size ^ "\t" ^ b2is f.return ^ "\t"
  ^ b2is f.return_int ^ "\t" ^ string_of_float f.extern ^ "\t" ^ b2is f.gvar
  ^ "\t" ^ b2is f.from_string ^ "\t" ^ b2is f.inside_loop ^ "\t"
  ^ b2is f.update_param_in_loop
  ^ "\t" ^ b2is f.use_ret_in_loop ^ "\t" ^ b2is f.update_param_itself ^ "\t"
  ^ b2is f.out_of_fun ^ "\t" ^ string_of_float f.finite ^ "\t"
  ^ string_of_float f.points_to
...
let add_constant exps feat =
  let rec const_condition = function
    | Const _ | SizeOf _ | SizeOfE _ -> true
...
let add_extern global node exps feat =
  if exps = [] then feat
    ...
    let has_extern e =
      ...
      in
      let locset =
        PowLoc.join (Val.pow_loc_of_val (Mem.lookup locset global.mem)) locset
      in
...
let add_gvar global node feat =
  let locset =
    Access.Info.accessof (AccessSem.accessof global node sem_fun global.mem)
  in
  let v = Mem.lookup locset global.mem in
  let locset =
    PowLoc.join (Val.pow_loc_of_val v)
...
let add_finite global node exps feat =
  ...
  in
  if
    (List.length exps = 3 && is_finite (List.nth exps 2))
  ...
  let is_cstring e =
    ...
    PowLoc.exists
      (fun loc ->
        ...
        let locset =
          PowLoc.fold
            (fun x -> BatSet.add (Loc.to_string x))
            locset BatSet.empty
        in
        let strlib =
          List.map
...
let add_use_ret_in_loop global node exps pid scc_list feat =
  let def =
    PowLoc.filter
      ...
      (Access.Info.defof (AccessSem.accessof global node sem_fun global.mem))
  in
  let b =
    List.exists
      (fun loop ->
...
let lib_feature global cfg trset =
  let scc_list =
    IntraCfg.get_scc_list cfg |> List.filter (fun x -> List.length x > 1)
  in
  IntraCfg.fold_node
    (fun intra_node trset ->
      match IntraCfg.find_cmd intra_node cfg with
