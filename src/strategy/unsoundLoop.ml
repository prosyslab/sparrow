...
let string_of_feature f =
  ...
  ^ string_of_bool f.array_content
  ^ "\n" ^ "conjunction : "
  ^ string_of_bool f.conjunction
  ^ "\n" ^ "single index : "
  ^ string_of_bool f.single_index
  ^ "\n" ^ "multi index : "
  ^ string_of_bool f.multi_index
  ^ "\n" ^ "out index : " ^ string_of_bool f.out_index ^ "\n" ^ "init index : "
  ^ string_of_bool f.init_index
  ...
  ^ string_of_bool f.prune_cond
  ^ "\n" ^ "extern : " ^ string_of_bool f.extern ^ "\n" ^ "gvar : "
  ^ string_of_bool f.gvar ^ "\n" ^ "finite_itv : "
  ^ string_of_bool f.finite_itv
  ^ "\n" ^ "finite_arr : "
  ^ string_of_bool f.finite_arr
  ^ "\n" ^ "close_left_arr_size : "
  ^ string_of_bool f.close_left_arr_size
  ^ "\n" ^ "close_left_arr_offset : "
  ^ string_of_bool f.close_left_arr_offset
  ^ "\n" ^ "points_to : "
...
let string_of_raw_feature f =
  b2is f.null ^ "\t" ^ b2is f.constant ^ "\t" ^ b2is f.array_content ^ "\t"
  ^ b2is f.conjunction ^ "\t" ^ b2is f.single_index ^ "\t" ^ b2is f.multi_index
  ^ "\t" ^ b2is f.out_index ^ "\t" ^ b2is f.init_index ^ "\t"
  ^ string_of_float f.exits ^ "\t" ^ string_of_float f.scc_size ^ "\t"
  ^ string_of_float f.diff_array_access
  ^ "\t" ^ string_of_float f.idx_pp ^ "\t" ^ string_of_float f.ptr_pp ^ "\t"
  ^ b2is f.prune_cond ^ "\t" ^ b2is f.extern ^ "\t" ^ b2is f.gvar ^ "\t"
  ^ b2is f.finite_itv ^ "\t" ^ b2is f.finite_arr ^ "\t"
...
let string_of_trset trset =
  BatMap.foldi
    (fun k v s -> s ^ k ^ "\t" ^ string_of_raw_feature v ^ "\n")
    trset ""
...
let add_cstring global cond_node cfg feat =
  ...
  let locset =
    ...
    if List.length pred1 = 1 && List.length pred2 = 1 && List.length pred3 = 1
    then
  ...
  PowLoc.fold
    (fun l feat ->
      ...
      let locset =
        Val.pow_loc_of_val (Mem.lookup (PowLoc.singleton l) global.mem)
      in
      let locset =
        PowLoc.fold (fun x -> BatSet.add (Loc.to_string x)) locset BatSet.empty
      in
      let strlib =
        List.map
          (fun x -> "__extern__" ^ x)
...
let add_gvar global cond feat =
  ...
  let locset = Access.Info.accessof access in
  let locset =
    PowLoc.join locset (Val.pow_loc_of_val (Mem.lookup locset global.mem))
  in
...
let add_null cond cfg feat =
  let rec null_condition = function
    ...
    | Lval _ -> true
    | BinOp (bop, e1, e2, _) when bop = Eq || bop = Ne ->
...
let add_finite_itv global cond_node feat =
  let access =
    PowLoc.remove Loc.null
      (Access.Info.accessof
         (AccessSem.accessof global cond_node sem_fun global.mem))
  in
  PowLoc.fold
    (fun loc feat ->
...
let add_index conds cfg scc feat =
  let rec collect_lv = function
    ...
    | Question (e1, e2, e3, _) ->
        BatSet.union
          (BatSet.union (collect_lv e1) (collect_lv e2))
          (collect_lv e3)
    | _ -> BatSet.empty
  in
  let lvset =
...
let add_out_index conds cfg scc feat =
  ...
  in
  let exits_nexts =
    List.fold_left (fun l n -> l @ IntraCfg.succ n cfg) [] exits
  in
  let qs =
    List.fold_left
      (fun qs node ->
...
let add_init global conds cfg scc feat =
  ...
  in
  let entry_prev =
    List.fold_left (fun l n -> l @ IntraCfg.pred n cfg) entries entries
  in
  let entry_prev =
    List.fold_left (fun l n -> l @ IntraCfg.pred n cfg) entry_prev entry_prev
  in
  let qs =
    List.fold_left
      (fun qs node ->
  ...
  List.fold_left
    (fun trset scc ->
      if List.length scc = 1 then trset
        ...
        in
        let loopid =
          try BatMap.find (IntraCfg.Node.id (List.hd branches)) loop_info
          with _ -> "unknown_loop"
        in
        let conds =
          List.map
        ...
        let feature =
          List.fold_left
            (fun feat cond_node ->
              let cond_exp =
                CilHelper.remove_cast (get_cond_exp cond_node cfg)
              in
              feat |> add_null cond_exp cfg |> add_constant cond_exp cfg
          ...
          |> add_exits conds |> add_scc_size scc |> add_points_to global conds
  ...
  List.fold_left
    (fun info stmt ->
      ...
      | If (_, tb, fb, _) ->
          ...
          |> generate_loop_info loopid tb.bstmts
...
let normalize trset =
  ...
  let max_scc =
    BatMap.fold
      (fun v max -> if v.scc_size > max then v.scc_size else max)
      trset 0.0
  in
  let max_points_to =
    BatMap.fold
      (fun v max -> if v.points_to > max then v.points_to else max)
      trset 0.0
  in
  ...
  let max_idx_pp =
    BatMap.fold
      (fun v max -> if v.idx_pp > max then v.idx_pp else max)
      trset 0.0
  in
  BatMap.map
    (fun feat ->
