...
module OctLoc = struct
  ...
  let to_string = function
    ...
    | Size l -> Allocsite.to_string l ^ ":size"

  let to_var : t -> Apron.Var.t = fun l -> to_string l |> Apron.Var.of_string
...
module PowOctLoc = struct
  ...
  let to_string x = if is_empty x then "{}" else to_string x
end

module Pack = PowOctLoc

module PackConf = struct
  ...
  let get_all_octlocs itvinputof =
    ItvAnalysis.Table.fold
      (fun _ mem pack ->
        ItvDom.Mem.foldi
          (fun x v pack ->
            if ItvDom.Val.pow_proc_of_val v <> PowProc.bot then pack
            else
              let plocs =
                ArrayBlk.allocsites_of_array (ItvDom.Val.array_of_val v)
              in
              pack
              |> opt
...
module Octagon = struct
  ...
  let interval_to_string itv =
    let lb =
      if Scalar.is_infty itv.Interval.inf = -1 then "-oo"
      else Scalar.to_string itv.Interval.inf
    in
    let ub =
      if Scalar.is_infty itv.Interval.sup = 1 then "+oo"
      else Scalar.to_string itv.Interval.sup
    in
    "[" ^ lb ^ ", " ^ ub ^ "]"
              ...
              (fun s y ->
                if x = y then
                  ...
                  if Interval.is_top itv then s
                    ...
                    s ^ Var.to_string x ^ " = " ^ interval_to_string itv ^ "\n"
                else
                  let sub =
  ...
  let itv_of_var : OctLoc.t -> t -> Itv.t =
   fun x o ->
    match o with
    | V o ->
        Abstract1.bound_variable man o
          (Apron.Var.of_string (OctLoc.to_string x))
        |> interval_to_itv
    | _ -> Itv.bot
  ...
  let set lv texpr oct =
    ...
    | V o ->
        ...
        if is_bot oct then bot else V o
    | Bot -> bot
  ...
  let join x y =
    match (x, y) with
    | x, Bot | Bot, x -> x
    | V x, V y -> V (Abstract1.join man x y)

  let meet x y =
    match (x, y) with
    | _, Bot | Bot, _ -> Bot
    | V x, V y -> V (Abstract1.meet man x y)
