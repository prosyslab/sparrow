...
module MakeCPO (A : AbsDom.SET) (B : AbsDom.CPO) = struct
    ...
    if x == y then true
      ...
      let rec loop e1 e2 =
        ...
        | Some (k1, v1), Some (k2, v2) ->
            ...
            if c = 0 then
              if B.le v1 v2 then loop (BatEnum.get enum1) (BatEnum.get enum2)
              else false
            else if c < 0 then
    ...
    if x == y then true
      ...
      let rec loop e1 e2 =
        ...
        | Some (k1, v1), Some (k2, v2) ->
            let c = A.compare k1 k2 in
            if c = 0 then
              if B.eq v1 v2 then loop (BatEnum.get enum1) (BatEnum.get enum2)
              else false
            else if c < 0 then
    ...
    if x == y then y
      ...
      let join' _ opt_v1 opt_v2 =
        ...
        | Some v1, Some v2 ->
            ...
            if B.eq joined_v B.bot then None else Some joined_v
      in
    ...
    if x == y then y
      ...
      let meet' _ opt_v1 opt_v2 =
        ...
        | Some v1, Some v2 ->
            ...
            if B.eq meeted_v B.bot then None else Some meeted_v
      in
    ...
    if x == y then x
      ...
      let widen' _ opt_v1 opt_v2 =
        ...
        | Some v1, Some v2 ->
            ...
            if B.eq widened_v B.bot then None else Some widened_v
      in
    ...
    if x == y then x
      ...
      let narrow' _ opt_v1 opt_v2 =
        ...
        | Some v1, Some v2 ->
            ...
            if B.eq narrowed_v B.bot then None else Some narrowed_v
      in
  ...
  let add : A.t -> B.t -> t -> t =
   fun k v x -> if B.eq v B.bot then BatMap.remove k x else BatMap.add k v x

  let weak_add : A.t -> B.t -> t -> t =
   fun k v x ->
    if B.eq v B.bot then x
    else
      BatMap.modify_def v k
        (fun orig_v ->
  ...
  let widen_add : A.t -> B.t -> t -> t =
   fun k v x ->
    if B.eq v B.bot then x
      ...
      BatMap.modify_def v k
        (fun orig_v -> if B.le v orig_v then orig_v else B.widen orig_v v)
        x
  ...
  *)
  ...
  let meet_big_small : t -> t -> t =
   fun big_x small_y ->
    let meet_a_map k v = B.meet v (find k big_x) in
    if le_small_big small_y big_x then small_y else mapi meet_a_map small_y
...
module MakeLAT (A : AbsDom.SET) (B : AbsDom.CPO) = struct
  module A = A
  module B = B
  module MapCPO = MakeCPO (A) (B)
  module PowA = PowDom.MakeCPO (A)
  ...
  let cardinal = function
    | V m -> MapCPO.cardinal m
    | Top -> raise (Failure "Error: cardinal")
  ...
  let eq x y =
    match (x, y) with
    | V x, V y -> MapCPO.eq x y
    | Top, Top -> true
    | _, _ -> false

  let filter p = function
    | V m -> V (MapCPO.filter p m)
  ...
  let meet x y =
    match (x, y) with
    | V x, V y -> V (MapCPO.meet x y)
    | _, Top -> x
    | Top, y -> y

  let widen x y =
    match (x, y) with
    | V x, V y -> V (MapCPO.widen x y)
    | _, Top | Top, _ -> top

  let narrow x y =
    match (x, y) with
    | V x, V y -> V (MapCPO.narrow x y)
    | Top, _ -> y
    | _, Top -> x
  ...
  (** The first argument of map,  function f,  should preserve the
      ...
      that [map f \[x |-> B.bot\]] and [map f \[ \]] return the same
      value.  *)
  let map f = function V m -> V (MapCPO.map f m) | Top -> Top

  let mapi f = function V m -> V (MapCPO.mapi f m) | Top -> Top
  ...
  (** The first argument of map,  function f,  should preserve the
      ...
      that [fold f \[x |-> B.bot\] acc] and [fold f \[ \] acc] return
      the same value.  *)
  let fold f x a = match x with V m -> MapCPO.fold f m a | Top -> a
