...
module Integer = struct
  ...
  let times x y =
    ...
    | PInf, Int n | Int n, PInf ->
        if n < 0 then MInf else if n > 0 then PInf else Int 0
    | MInf, Int n | Int n, MInf ->
        if n < 0 then PInf else if n > 0 then MInf else Int 0

  let divide x y =
    match (x, y) with
    | Int n1, Int n2 ->
        if n2 = 0 then invalid_arg "itv.ml: Integer.divide (_, 0)"
        else Int (n1 / n2)
    | PInf, PInf | MInf, MInf -> PInf
    | MInf, PInf | PInf, MInf -> MInf
    | MInf, Int n ->
        if n < 0 then PInf
        else if n > 0 then MInf
        else invalid_arg "itv.ml: Integer.divide (-oo, 0)"
    | PInf, Int n ->
        if n < 0 then MInf
...
let upper_integer = function
  | V (_, x) -> x
  | _ -> invalid_arg "Itv.upper_integer"

let lower_integer = function
  | V (x, _) -> x
...
let to_json itv = `String (to_string itv)
...
let absolute = function
  ...
  | V (l, u) ->
      ...
      else if Integer.le l Integer.zero && Integer.le Integer.zero u then
        V (Integer.zero, Integer.max (Integer.absolute l) u)
...
let eq x y =
  if is_bot x && is_bot y then true
  else if is_bot x || is_bot y then false
  else
    match (x, y) with
...
let join x y =
  ...
  else if is_bot x then normalize y
  else if is_bot y then normalize x
  else
    match (x, y) with
...
let meet x y =
  ...
  else if is_bot x then Bot
  else if is_bot y then Bot
  else
    match (x, y) with
    | V (l1, u1), V (l2, u2) ->
...
let narrow x y =
  ...
  else if is_bot x then invalid_arg "itv.ml: narrow(bot, _)"
  else
    match (x, y) with
    | V (l1, u1), V (l2, u2) ->
...
let open_right x =
  if is_bot x then false else match x with V (_, PInf) -> true | _ -> false

let close_left x =
  if is_bot x then false else match x with V (Int _, _) -> true | _ -> false
...
let is_finite x =
  ...
  else match x with V (Int _, Int _) -> true | _ -> false
...
let is_negative x =
  if is_bot x then false
    ...
    match x with V (MInf, _) -> true | V (Int x, Int _) -> x < 0 | _ -> false
...
let diff x =
  if is_bot x then 0 else match x with V (Int l, Int u) -> u - l | _ -> 0
...
let divide x y =
  if is_bot x || is_bot y then Bot
  else if le (V (Int 0, Int 0)) y then top
  else
    match (x, y) with
    | V (l1, u1), V (l2, u2) ->
...
let l_and x y =
  if is_bot x || is_bot y then Bot
  else if eq false_itv x || eq false_itv y then false_itv
  else if (not (le false_itv x)) && not (le false_itv y) then true_itv
  else unknown_bool_itv
...
let eq_itv x y =
  if is_bot x || is_bot y then Bot
    ...
    match (x, y) with
    | V (Int l1, Int u1), V (Int l2, Int u2) when l1 = u1 && u1 = l2 && l2 = u2
      ->
        true_itv
...
let l_not x =
  ...
  else if eq false_itv x then true_itv
  else if le false_itv x then unknown_bool_itv
  else false_itv
    ...
    | _ ->
        ...
        if CilHelper.is_unsigned from_typ && CilHelper.is_unsigned to_typ then
          if from_size <= to_size then itv
            ...
            Integer.le (upper_integer itv) (upper_integer (itv_of_type to_typ))
          then itv
        ...
        then
          ...
          CilHelper.is_unsigned from_typ && not (CilHelper.is_unsigned to_typ)
        then
          if from_size < to_size then itv
          else if
            Integer.le (upper_integer itv) (upper_integer (itv_of_type to_typ))
        ...
        else if from_size <= to_size then itv
        else if
          Integer.le (upper_integer itv) (upper_integer (itv_of_type to_typ))
        then itv
        else top (* possibly overflow *)
  else if CilHelper.is_unsigned to_typ then absolute itv
  else itv
