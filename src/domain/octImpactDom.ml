...
module AbsOct = struct
  ...
  type t = Bot | V of G.t * PowOctLoc.t * PowOctLoc.t
  ...
  let remove x y = function
    | Bot -> bot
    | V (g, c, d) -> V (G.remove_edge g x y, c, d)

  (* idx - size <= * *)
  let check idx size o =
    match o with
    | Bot -> false
    | V (g, _, _) ->
        let path_checker = Check.create g in
  ...
  let set_const x = function
    | Bot -> bot
    | V (g, c, d) -> V (g, PowOctLoc.add x c, d |> PowOctLoc.add x)

  let weak_set_const x = function
    | Bot -> bot
    | V (g, c, d) -> V (g, c, d |> PowOctLoc.add x)

  (* x := y + c *)
  let set_variable x y = function
    | Bot -> bot
    | V (g, c, d) ->
  ...
  let weak_set_variable x y = function
    | Bot -> bot
    | V (g, c, d) ->
        if OctLoc.compare x y = 0 then V (g, c, d)
        else if PowOctLoc.mem y d then V (g, c, PowOctLoc.add x d)
          (* |> closure *)
        else V (g, c, d)
  ...
  let forget x = function
    | Bot -> bot
    | V (g, c, d) ->
        V (G.remove_vertex g x, PowOctLoc.remove x c, d |> PowOctLoc.add x)

  (* x - y <= c *)
  let assume x y = function
    | Bot -> bot
    | V (g, c, d) ->
        if OctLoc.compare x y = 0 then V (g, c, d)
  ...
  let join x y =
    ...
    | V (x, c1, d1), V (y, c2, d2) ->
        ...
        let newg =
          G.fold_edges
            (fun s d y -> if G.mem_edge x s d then y else G.remove_edge y s d)
            y y
        in
        V (newg, PowOctLoc.meet c1 c2, PowOctLoc.join d1 d2)
  ...
  let meet x y =
    ...
    | V (x, c1, d1), V (y, c2, d2) ->
        ...
        in
        let newg = G.fold_edges (fun s d y -> G.add_edge y s d) x y in
...
module Relation = struct
  ...
  let add_absoct aoct g =
    AbsOct.fold (fun src dst g -> G.add_edge g src dst) aoct g
