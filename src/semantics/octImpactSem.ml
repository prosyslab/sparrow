...
let can_strong_update mode global pid =
  match mode with Weak -> false | Strong -> not (Global.is_rec pid global)

let can_strong_update_octloc mode global lvs =
  match (mode, lvs) with
  | Weak, _ -> false
  | Strong, lvs' ->
...
let rec set mode global ptrmem pid lv_set e mem =
  ...
  if try Cil.isIntegralType (Cil.typeOf e) with _ -> false then
    ...
    | (BinOp (bop, Lval lval, Const _, _) | BinOp (bop, Const _, Lval lval, _))
...
let rec prune mode global ptrmem pid exp mem =
  ...
  | Some
    ...
    ->
      prune mode global ptrmem pid
        (Cil.BinOp (bop, Cil.Lval x, Cil.Lval y, t))
        mem
  | Some (Cil.BinOp (bop, Cil.Lval x, Cil.Lval y, t)) when bop = Gt || bop = Ge
    ->
      prune mode global ptrmem pid
        (Cil.BinOp (CilHelper.rev_binop bop, Cil.Lval y, Cil.Lval x, t))
        mem
  | _ -> mem
...
let model_strdup mode pid node lvo exps ptrmem (mem, global) =
  ...
  | Some _, str :: _ ->
      ...
      in
      let rv_set =
        ItvSem.eval pid str ptrmem |> ItvDom.Val.allocsites_of_val
        |> PowOctLoc.of_sizes
      in
      let o =
        if
...
let run_cmd mode node cmd ptrmem (mem, global) =
  ...
  | IntraCfg.Cmd.Csalloc (l, s, _) ->
      ...
      alloc mode global ptrmem pid lv ptrs e mem
  | IntraCfg.Cmd.Cassume (e, _, _) -> prune mode global ptrmem pid e mem
