...
let lval_to_texpr pid packconf pack lv premem mem =
...
let rec exp_to_texpr pid packconf pack e ptrmem mem =
  ...
  | Cil.BinOp (bop, e1, e2, _) ->
      binop_to_texpr pid packconf pack bop e1 e2 ptrmem mem
  | _ -> []
...
and uop_to_texpr pid packconf pack uop e ptrmem mem =
  ...
  | Cil.Neg ->
      List.map
        (fun x -> Texpr1.Unop (Texpr1.Neg, x, Texpr1.Int, Texpr1.Near))
        e_list
  | Cil.BNot | Cil.LNot ->
      List.map (fun _ -> Texpr1.Cst (Coeff.Interval Interval.top)) e_list
      ...
      List.fold_left
        (fun l y ->
          ...
          | Cil.PlusA | Cil.MinusA | Cil.Mult | Cil.Div ->
              Texpr1.Binop (bop_to_texpr bop, x, y, Texpr1.Int, Texpr1.Near)
              :: l
...
let cond_to_texpr pid packconf pack lv bop e ptrmem mem =
  exp_to_texpr pid packconf pack e ptrmem mem |> List.map (normalize bop lv)
 ...
 * ************************** *)
let set mode global ptrmem packconf pid lv_set e mem =
...
let rec prune mode global ptrmem packconf pid exp mem =
  match
    exp |> CilHelper.remove_cast |> CilHelper.remove_coeff
    |> CilHelper.make_cond_simple
  with
  | None -> mem
    ...
    when bop = Lt || bop = Gt || bop = Le || bop = Ge || bop = Eq || bop = Ne ->
      ...
      PowOctLoc.fold
        (fun lv l ->
          ...
          let old_oct = lookup pack mem in
          let texpr_list =
            cond_to_texpr pid packconf pack lv bop e ptrmem mem
          in
          List.fold_left
            (fun l (texpr, typ) ->
  ...
  | Some (Cil.UnOp (LNot, Lval x, t)) ->
      prune mode global ptrmem packconf pid
        (Cil.BinOp (Eq, Lval x, Cil.zero, t))
        mem
  | Some (Lval x) ->
      prune mode global ptrmem packconf pid
        (Cil.BinOp (Gt, Lval x, Cil.zero, Cil.intType))
        mem
  | _ -> mem
      ...
      let mem =
        PowOctLoc.fold
          (fun lv l ->
            ...
            let old_oct = lookup pack mem in
            let texpr_list =
              strlen_texpr_set pid packconf pack str ptrmem mem
            in
            List.fold_left
              (fun l texpr -> (lv, pack, Octagon.set lv texpr old_oct) :: l)
              l texpr_list)
      ...
      let l =
        PowOctLoc.fold
          (fun lv l ->
            ...
            let old_oct = lookup pack mem in
            let texpr_list =
              strlen_texpr_prune pid packconf pack lv str ptrmem mem
            in
            List.fold_left
              (fun l (texpr, typ) ->
                (lv, pack, Octagon.prune lv texpr typ old_oct) :: l)
...
let sparrow_arg mode packconf pid exps ptrmem (mem, global) =
  ...
  | Cil.Lval argc :: Cil.Lval argv :: _ ->
      ...
      let argv_a =
        Allocsite.allocsite_of_ext (Some "argv")
        |> OctLoc.of_size |> PowOctLoc.singleton
      in
      let arg_a =
        Allocsite.allocsite_of_ext (Some "arg")
        |> OctLoc.of_size |> PowOctLoc.singleton
      in
      mem
      |> forget mode global packconf pid argc_lv
      |> prune mode global ptrmem packconf pid
           (Cil.BinOp (Cil.Ge, Cil.Lval argc, Cil.one, Cil.intType))
...
let strdup_texpr pid packconf pack exp ptrmem mem =
  let set =
    ItvSem.eval pid exp ptrmem |> ItvDom.Val.allocsites_of_val
    |> PowOctLoc.of_sizes
  in
...
let run_cmd mode packconf node cmd ptrmem (mem, global) =
  ...
  | IntraCfg.Cmd.Cassume (e, _, _) ->
      prune mode global ptrmem packconf pid e mem
...
let accessof ?(locset = PackConf.empty) global node f mem =
  ...
  let itv_use, itv_def =
    ptrmem
    |> ItvAccessSem.accessof global node
         (ItvSem.run AbsSem.Strong ItvSem.Spec.empty)
    |> tuple
 ...
 * ********************** *)
let check_bo pid packconf a offset idx ptrmem mem =
