...
let alloc analysis n lv e ploc v relations =
  if analysis = Spec.Pre then
    ...
    PowLoc.fold (fun l ->
        PowLoc.fold (fun r ->
          add_alloc n lv e l r) rvset) ploc relations
  else
    relations
...
let add_set icfg node l r prov relations =
  ...
  | IntraCfg.Cmd.Cset (lv, e, _) ->
    ...
    let rel = Rel.PointsTo.make_assign_set node lv e l r pairset |> Rel.points_to in
    Rel.Set.add rel relations
  | _ -> relations

let set analysis icfg node e ploc v prov relations =
  if analysis = Spec.Pre
  && Cil.isPointerType @@ Cil.unrollTypeDeep @@ Cil.typeOf e then
    let rvset = ItvDom.Val.all_locs v |> PowLoc.remove Loc.null in
    PowLoc.fold (fun l relations ->
        PowLoc.fold (fun r relations ->
            add_set icfg node l r prov relations) rvset relations)
      ploc relations
  else
    relations
...
let all_locs prov =
  G.fold_vertex (fun v l ->
      if Loc.equal v Loc.dummy then l
      else v::l) prov []
...
let call analysis icfg node arg_lvars_set prov_list relations =
  if analysis = Spec.Pre then
    ...
    BatSet.fold (fun arg_lvars relations ->
      List.fold_left2 (fun relations formal prov ->
            let locs = all_locs prov in
            List.fold_left (fun relations l ->
                let rel = Rel.Bind.make l formal in
                Rel.bind rel |> flip Rel.Set.add relations) relations locs)
        relations arg_lvars prov_list) arg_lvars_set relations
  else
    relations
...
let print relations =
  ...
  Rel.Set.iter (function
      | Rel.PointsTo (x, y, _) ->
        F.fprintf fmt_ptr "%a\t%a\n" Loc.pp x Loc.pp y
      | Rel.Bind (x, y) ->
        F.fprintf fmt_bind "%a\t%a\n" Loc.pp x Loc.pp y
    ) relations;
  close_out oc_prov;
  close_out oc_bind;
  close_out oc_ptr
