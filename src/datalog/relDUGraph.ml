...
let optimize_inter_edge global old_g =
  G.fold_edges_e (fun src dst locset new_g ->
      if not (InterCfg.is_callnode src global.Global.icfg)
      && not (InterCfg.is_callnode dst global.Global.icfg)
      && not (InterCfg.is_exit src)
      && not (InterCfg.is_exit dst) then
...
let reachability2 global alarms g =
  ...
  let rec fix works results g =
    if ReachingDef.is_empty works then results
      ...
      G.fold_pred (fun p (works, results) ->
          ...
          if G.PowLoc.mem use locs_on_edge then
            if ReachingDef.mem (p, use) results then (works, results)
              ...
              if G.PowLoc.mem use defs_pred then
                ...
                (ReachingDef.add (p, use) works, ReachingDef.add (p, use) results)
          else if p = InterCfg.start_node then
            (works, ReachingDef.add (p, use) results)
          else
            (works, results)) g node (works, results)
  ...
  let works =
    List.fold_left (fun set alarm ->
        ...
        G.PowLoc.fold (fun x -> ReachingDef.add (node, x)) uses set) ReachingDef.empty alarms
  in
  let reachable_from_node = fix works works g in
  let reachable_from_node = ReachingDef.fold (fun x -> PowNode.add (fst x)) reachable_from_node PowNode.empty
  in
  G.fold_node (fun n g -> if PowNode.mem n reachable_from_node then g else G.remove_node n g) g g
...
let print global dug alarms =
  ...
  let (true_branch, false_branch) =
    InterCfg.fold_cfgs (fun pid cfg (true_branch, false_branch) ->
        IntraCfg.fold_node (fun node (true_branch, false_branch) ->
            let succs = IntraCfg.succ node cfg in
            if List.length succs = 2 then
              (PowNode.add (InterCfg.Node.make pid (List.nth succs 1)) true_branch,
               PowNode.add (InterCfg.Node.make pid (List.nth succs 0)) false_branch)
